...
var PARSER_PATH = path.resolve(__dirname, '../vendor/stanford-parser');
var MODEL_PATH = 'models/englishPCFG.caseless.ser.gz';
...
StanfordParser.parser = function() {
    if (java && !StanfordParser._parser) {
        java.classpath.push(path.join(PARSER_PATH, 'stanford-parser.jar'));
        StanfordParser._parser = java.callStaticMethodSync(
            'edu.stanford.nlp.parser.lexparser.LexicalizedParser',
            'loadModel',
            path.join(PARSER_PATH, MODEL_PATH)
        );
    }
