...
var config = {
    ...
    invalidCharsInTitle: {invalidChars: /[^\w(), '"`\-:\./~\[\]*$={}&;#+@<>%^]/, type: Error.ERROR},
    ...
    references: {alwaysCheck: false, type: Error.ERROR},
...
module.exports = function(cfg, defaultConfig) {
    var c = typeof(defaultConfig) === 'undefined' ? clone(config) : defaultConfig;
    var overwrite = function(what, data) {
        if (data instanceof Object && !(data instanceof RegExp)) {
            for (var i in data) {
