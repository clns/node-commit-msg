...
var config = {
    ...
    // the order in which you specify multiple references here does matter; refs that
    // appear first will be validated before other refs (e.g. when you have one ref that
    // is used more often than another one, this will improve performance).
    ...
    // set 'allow' to false to error out if these are found (useful for server-side validation);
    // set 'validateSquash' to false to skip validation for this commit;
    // set 'validateFixup' to true to validate fixup! commits (by default these are not validated);
    // 'validateSquash' and 'validateFixup' only work if 'allow' is true;
    // set 'squashFixup' to false to disable looking for these commits.
    squashFixup: {allow: true, validateSquash: true, validateFixup: false, type: Error.ERROR},
    // configurations for the 'update' hook
    updateHook: {
        // how many failed commits to display when validation fails on the server;
        // check out the 'bin/update' hook for more details.
        showFirst: 2
    }
};
