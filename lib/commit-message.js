...
CommitMessage.parse = function(message, config) {
    var msg = new CommitMessage(message, Config( resolveConfig(config) ));
    msg.validate();
    return msg;
}

CommitMessage.parseFromFile = function(file, config) {
    var msg = fs.readFileSync(file, { encoding: 'utf8' });
    return CommitMessage.parse(msg, config);
}
...
function resolveConfig(cfg) {
    ...
    if (cfg && typeof(cfg) != 'object') {
        ...
        cfg = getPackageJsonConfig(typeof(cfg) == 'string' ? cfg : __dirname);
    }
    return cfg;
}
