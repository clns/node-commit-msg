...
var config = {
    ...
    // parse subject and check for non-imperative verbs
    // elements: 1 - enable/disable, 2 - always check: by default (false), it
    // will only run this check if the message is valid so far
    imperativeVerbsInTitle: [true, false, Error.ERROR],
...
CommitMessage.prototype._imperativeVerbsInTitle = function(t) {
    ...
    if (cfg.imperativeVerbsInTitle[0] && ( cfg.imperativeVerbsInTitle[1] || !this.hasErrors() )) {
            ...
            if (parser.hasVerb()) {
                ...
                var checkImperative = function(verb) {
                    ...
                    if (matches) {
                        ...
                        cfg.imperativeVerbsInTitle[2], [1, index+1+colOffset]);
                ...
                if (baseNode) {
                    baseNode.children.every(function(child) {
                        ...
                        return verbs.every(checkImperative);
                    });
                }
            }
            return !hasError;
