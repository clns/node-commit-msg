...
function CommitMessage(message) {
    ...
    if (!isSemver && cfg.capitalized && !/^[A-Z]/.test(t)) {
        err('Commit message should start with a capitalized letter');
    }

    if (/\s\s/.test(t)) {
        err('First line (summary) contains invalid whitespace');
    }

    if (/[\.\s]$/.test(t)) {
        err('First line (summary) should not end with a period or whitespace');
    }
