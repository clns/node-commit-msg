...
CommitMessage.prototype.validate = function() {
    ...
    if (cfg.strictTypes[0] && cfg.strictTypes[0].test(t)) {
        log(util.format('Invalid type %s', t.match(cfg.strictTypes[0])[1].trim()),
    ...
    if (t.length > cfg.titleMaxLineLength[0]) {
        log(util.format('First line (subject) should not exceed %d characters',
    ...
    if ((index = t.search(/\s\s/)) !== -1) {
        log('First line (subject) contains invalid whitespace',
        Error.ERROR, [1, index+1+colOffset]);
    }

    if (/[\.\s]$/.test(t)) {
        log('First line (subject) should not end with a period or whitespace',
        Error.ERROR, [1, t.length+colOffset]);
    }

    if (cfg.invalidCharsInTitle[0] && (index = t.search(cfg.invalidCharsInTitle[0])) !== -1) {
        log('First line (subject) contains invalid characters',
