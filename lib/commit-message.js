...
var config = {
    ...
    imperativeVerbsInTitle: [false, Error.ERROR],
    imperativeVerbsInBody: [false, Error.WARNING],
...
CommitMessage.prototype._imperativeVerbsInTitle = function(t) {
    ...
    if (cfg.imperativeVerbsInTitle[0]) {
        var parser = Parser.parseSentencesSync([t])[0];
        if (parser.hasVerb()) {
            // If the commit message contains at least 1 verb, continue
            t = 'This patch will ' + t;
            parser = Parser.parseSentencesSync([t])[0];

            // get node that contains the first level of verbs as childre
            var node = parser.penn.getChildrenWithValue(/^S/)[0]
            .getChildrenOfType(/VP/)[0];
        }
    }
}
